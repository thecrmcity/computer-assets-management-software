...
if(isset($_POST['addaset']))
{
	...
	if($rss == true)
	{
		header('Location:electronic.php?act=create');
	}
}
...
if(isset($_POST['submitdata']))
{
	...
	switch($assets)
	{
		...
		if($tvnum <= 0)
		{
			...
			$alts = "INSERT INTO `dis_electronicdata`(`dis_code`, `dis_comcode`, `dis_pdate`, `dis_expridate`, `dis_duration`, `dis_pname`, `dis_psubname`, `dis_status`) VALUES ('$vcnt','EL-1','$pdate','$effDate','$warrdur','$assets','$remarks','1')";
		...
		{
			header('Location:electronic.php?act=ent');
		}
		else
		{
	...
	<div class="sidebar">
		<div class="sidebar-set">
			<div class="fistmenu mb-4">
				<p>Item Creation</p>
					<ul class="nav flex-column">
						<li class="navitem"><a href="electronic.php?act=create" class="navlink">Create New <i class="	fa fa-angle-double-right"></i></a></li>
			...
			<div class="fistmenu">
				<p>Item Inventory</p>
					<ul class="nav flex-column">
						<li class="navitem"><a href="electronic.php?act=ent" class="navlink"> Electronics Entry <i class="fa fa-angle-double-right"></i></a></li>
			...
			<div class="fistmenu mt-4">
				<p>Electronic Items</p>
					<ul class="nav flex-column">
						...
						<?php
							$sql = "SELECT DISTINCT dis_remarks FROM `dis_electronic` WHERE dis_assets='Electronics'";
							$res = mysqli_query($conn,$sql);
							while($row = mysqli_fetch_array($res))
							{
								?>
								<li class="navitem"><a href="electronic.php?act=<?php echo $row['dis_remarks'];?>" class="navlink"> <?php echo $row['dis_remarks'];?> <i class="fa fa-angle-double-right"></i></a></li>
								<?php
							}
			...
			<?php
				...
				<table class="table table-bordered table-striped table-hover">
					...
					<tbody>
						<?php
						$nums = 1;
						$mvql = "SELECT * FROM `dis_electronic` WHERE dis_assets='Electronics'";
						...
						while($mrow = mysqli_fetch_array($mvres))
						{
							...
							<td><?php echo $nums;?></td>
								...
								?></td>
								<td><a href="elhistory.php?id=<?php echo $mrow['dis_code'];?>&set=<?php echo 'Electronics';?>" class="histbnt">History</a></td>
								<td><a href="elsupply.php?id=<?php echo $mrow['dis_code'];?>&set=<?php echo 'Electronics';?>" class="suppbnt">Issue</a></td>
				...
				<table class="table table-bordered table-striped table-hover">
					...
					<tbody>
						...
						$nums = 1;
						$mvql = "SELECT * FROM `dis_alldata` WHERE dis_psubname='$act' AND dis_pname='Electronics'";
						$mvres = mysqli_query($conn,$mvql);
						while($mrow = mysqli_fetch_array($mvres))
						{
							?>
				...
				else
				{
                	...
                	<td colspan=4 class="text-center bg-dark text-white font-weight-bold">
                		...
                		<select class="" name="est">
                			...
                			<?php
                				$tsql = "SELECT DISTINCT dis_psubname FROM `dis_electronicdata` WHERE dis_pname='Electronics'";
            	...
            	<?php
						$osql = "SELECT * FROM `dis_electronicdata` WHERE dis_pname='Electronics' AND dis_status IN ('1','2','3')";
					$ores = mysqli_query($conn,$osql);
					$onum = mysqli_num_rows($ores);

					$esql = "SELECT * FROM `dis_electronicdata` WHERE dis_pname='Electronics' AND dis_status IN ('1')";
					$eres = mysqli_query($conn,$esql);
					$enum = mysqli_num_rows($eres);

					$dsql = "SELECT * FROM `dis_electronicdata` WHERE dis_pname='Electronics' AND dis_status IN ('2')";
					$dres = mysqli_query($conn,$dsql);
					$dnum = mysqli_num_rows($dres);

					$osqlm = "SELECT * FROM `dis_electronicdata` WHERE dis_pname='Electronics' AND dis_status IN ('3')";
					...
					$est = @$_GET['est'];
					$estnt = "SELECT * FROM `dis_electronicdata` WHERE dis_pname='Electronics' AND dis_status IN ('1','2','3') AND dis_psubname='$est'";
					$estntre = mysqli_query($conn,$estnt);
					$estntnum = mysqli_num_rows($estntre);

					$spl = "SELECT * FROM `dis_electronicdata` WHERE dis_pname='Electronics' AND dis_status='2' AND dis_psubname='$est'";
					$ospl = mysqli_query($conn,$spl);
					$osnum = mysqli_num_rows($ospl);

					$balq = "SELECT * FROM `dis_electronicdata` WHERE dis_pname='Electronics' AND dis_status='1' AND dis_psubname='$est'";
					$balrs = mysqli_query($conn,$balq);
					$balnm = mysqli_num_rows($balrs);

					$rpql = "SELECT * FROM `dis_electronicdata` WHERE dis_pname='Electronics' AND dis_status='3' AND dis_psubname='$est'";
					...
					<div class="row">
						<div class="col-lg-6 col-md-6">
								...
								<tbody>
									<?php
										if(isset($_GET['est']))
										{
											$tnum=1;
												$est = $_GET['est'];
							$msql = "SELECT COUNT(dis_issueto) AS nums, dis_branch FROM dis_electronicdata WHERE dis_pname='Electronics' AND dis_psubname='$est' AND dis_status='2' GROUP BY dis_branch";
							...
							while($mrow = mysqli_fetch_array($mres))
							{
								...
								<tr>
								<td><?php echo $tnum;?></td>
								<td><a href="branch.php?pro=<?php echo $mrow['dis_branch'];?>&pd=Electronics" class="tdlink"><?php echo $mrow['dis_branch'];?></td>
								...
								$tnum++;
									...
									{
										$tnum=1;
							$msql = "SELECT COUNT(dis_issueto) AS nums, dis_branch FROM dis_electronicdata WHERE dis_pname='Electronics' AND dis_status='2' GROUP BY dis_branch";
							$mres = mysqli_query($conn,$msql);
							while($mrow = mysqli_fetch_array($mres))
							{
								?>
									...
									if(isset($_GET['est']))
										{
											$tnum=1;
											$est = $_GET['est'];
							$msql = "SELECT COUNT(dis_issueto) AS nums, dis_location FROM dis_electronicdata WHERE dis_pname='Electronics' AND dis_psubname='$est' AND dis_status='2' GROUP BY dis_location";
							$mres = mysqli_query($conn,$msql);
							while($mrow = mysqli_fetch_array($mres))
							{
								?>
								<tr>
								<td><?php echo $tnum;?></td>
								<td><a href="location.php?pro=<?php echo $mrow['dis_location'];?>&pd=Electronics" class="tdlink"><?php echo $mrow['dis_location'];?></td>
								<td><?php echo $mrow['nums'];?></td>
						...
						{
							$tnum=1;
							$msql = "SELECT COUNT(dis_issueto) AS nums, dis_location FROM dis_electronicdata WHERE dis_pname='Electronics' AND dis_status='2' GROUP BY dis_location";
							$mres = mysqli_query($conn,$msql);
							while($mrow = mysqli_fetch_array($mres))
							{
								?>
									...
									if(isset($_GET['est']))
										{
											$tnum=1;
							$est = $_GET['est'];
							$msql = "SELECT COUNT(dis_building) AS nums, dis_building FROM dis_electronicdata WHERE dis_pname='Electronics' AND dis_psubname='$est' AND dis_status='2' GROUP BY dis_building";
							$mres = mysqli_query($conn,$msql);
							while($mrow = mysqli_fetch_array($mres))
							{
								?>
								<tr>
								<td><?php echo $tnum;?></td>
								<td><a href="building.php?pro=<?php echo $mrow['dis_building'];?>&pd=Electronics" class="tdlink"><?php echo $mrow['dis_building'];?></td>
								<td><?php echo $mrow['nums'];?></td>
						...
						else
						{
							...
							$msql = "SELECT COUNT(dis_building) AS nums, dis_building FROM dis_electronicdata WHERE dis_pname='Electronics' AND dis_status='2' GROUP BY dis_building";
							$mres = mysqli_query($conn,$msql);
							while($mrow = mysqli_fetch_array($mres))
							{
								?>
									...
									<tbody>
										<?php
											if(isset($_GET['est']))
											{
												$tnum=1;
												$est = $_GET['est'];
							$msql = "SELECT COUNT(dis_issueto) AS nums, dis_issueto FROM dis_electronicdata WHERE dis_pname='Electronics' AND dis_psubname='$est' AND dis_status='2' GROUP BY dis_issueto";
							$mres = mysqli_query($conn,$msql);
							while($mrow = mysqli_fetch_array($mres))
							{
								?>
								<tr>
								<td><?php echo $tnum;?></td>
								<td><a href="process.php?pro=<?php echo $mrow['dis_issueto'];?>&pd=Electronics" class="tdlink"><?php echo $mrow['dis_issueto'];?></td>
								<td><?php echo $mrow['nums'];?></td>
								...
								$tnum++;
											...
											{
												$tnum=1;
												$msql = "SELECT COUNT(dis_issueto) AS nums, dis_issueto FROM dis_electronicdata WHERE dis_pname='Electronics' AND dis_status='2' GROUP BY dis_issueto";
							$mres = mysqli_query($conn,$msql);
							while($mrow = mysqli_fetch_array($mres))
							{
								?>
...
<section id="stockinvnt" class="popupform">
		<div class="popform animate">
			<h5 class="stocktxt text-center">Electronics Inventory</h5>
			<form class="" method="post" action="">
							<div class="form-group row">
								...
								<div class="col-md-6">
                                	...
                                	<option value="" disabled="" selected="">Select Electronic</option>
                                	<?php

					...
					<tbody>
						<?php
							if(isset($_GET['pro']) AND isset($_GET['pd']))
							{
								...
								$sql = "SELECT * FROM `dis_alldata` WHERE dis_building='$pro' AND dis_pname='$pd' AND dis_status='2'";
								$res = mysqli_query($conn,$sql);
							...
							else
							{
								$nums =1;
								$pro = $_GET['pro'];
								$sql = "SELECT * FROM `dis_alldata` WHERE dis_pname in ('New Computer Set','Old Computer Set') AND dis_building='$pro' AND dis_status='2'";
								$res = mysqli_query($conn,$sql);
								while($mrow = mysqli_fetch_array($res))
								{
								?>
